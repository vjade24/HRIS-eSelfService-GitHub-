...
namespace HRIS_Common
{
	...
	class CommonDB
	{
        ...
        // Date Created :   09/07/2018
        //***********************************************************************************************************************
        public DataTable RetrieveDataHRD(string sp_script, string parmagr1, string parmvalue1)
        {
            //try
            //{
            DataTable dt = new DataTable();
            string connStr = System.Configuration.ConfigurationManager.ConnectionStrings["cnHRISHRD"].ConnectionString;
            using (SqlConnection cn = new SqlConnection(connStr))
            {
                SqlCommand cmd = new SqlCommand(sp_script, cn);
                cmd.CommandType = CommandType.StoredProcedure;
