...
{
    public class rSSDTRPrintingController : Controller
    {
            ...
            )
        {
            ...
            par_department_code = db_ats.sp_get_empl_employment_type(par_empl_id).ToList()[0].department_code;
            Session["department_dtr"] = par_department_code;
            //var session_employment_type = Session["employment_type"].ToString();
            ...
            //var empl_dtr_hdr_tbl_check = db_ats.empl_dtr_dtl_tbl.Where(a => a.empl_id == par_empl_id && a.dtr_date.Month == month_int && a.dtr_date.Year == year_int).ToList();

            //if (empl_dtr_hdr_tbl_check.Count <= 0)
            //{
            //    sp_generate_empl_dtr = db_ats.sp_generate_empl_dtr(par_year, par_month, par_empl_id, par_view_type, par_department_code, session_employment_type, session_user_id);
            //    db_ats.SaveChanges();
            //}
            
           
            sp_report = db_ats.sp_dtr_rep(par_year, par_month, par_empl_id, par_view_type, par_department_code, session_user_id).ToList();
            ...
            )
        {
            try
            {
                ...
                par_employment_type = db_ats.sp_get_empl_employment_type(par_empl_id).ToList()[0].employment_type;
                par_department_code = db_ats.sp_get_empl_employment_type(par_empl_id).ToList()[0].department_code;
