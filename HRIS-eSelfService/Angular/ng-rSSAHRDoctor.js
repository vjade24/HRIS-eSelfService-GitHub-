ng_selfService_App.controller("rSSAHRDoctor_ctrlr", function ($scope, $compile, $http, $filter, Upload, $timeout) {
    var s = $scope
    var h = $http
    ...
    function init() {
        ...
        //**********************************************
        h.post("../rSSAHRDoctor/InitializeData").then(function (d) {
                        ...
                        "mRender": function (data, type, full, row) {
                            ...
                            if (full["dtr_order_no"] == "") {
                                ...
                                return_html = '<center><div class="btn-group">' +
                                    '<button type="button" ng-show="ShowEdit" class="btn btn-primary btn-sm" ng-click="btn_edit_action(' + row["row"] + ')" data-toggle="tooltip" data-placement="top" title="Only Viewing of details because it already has approved transmittal.">  <i class="fa fa-pencil"></i> Update</button >' +
                                    '</div></center>';
                            }
                                ...
                                return_html = '<center><div class="btn-group">' +
                                    '<button type="button" ng-show="ShowEdit" class="btn btn-primary btn-sm" ng-click="btn_edit_action(' + row["row"] + ')" data-toggle="tooltip" data-placement="top" title="Only Viewing of details because override already in-process/approved.">  <i class="fa fa-pencil"></i>Update</button >' +
                                    '</div></center>';
                            }
                            else {
                                return_html = '<center><div class="btn-group">' +
                                    '<button type="button" ng-show="ShowEdit" class="btn btn-warning btn-sm" ng-click="btn_edit_action(' + row["row"] + ')" data-toggle="tooltip" data-placement="top" title="Override Daily Time Records">  <i class="fa fa-edit"></i> Update</button >' +
                                    '</div></center>';
                            }
    ...
    s.FilterPageGrid = function () {
        try {
            h.post("../rSSAHRDoctor/FilterPageGrid", {
    ...
    s.ddl_dept_chane = function () {
        try {
            h.post("../rSSAHRDoctor/DepartmentFilter",
                {
                    p_dept_code: $("#ddl_dept option:selected").val()
                }).then(function (d) {
                    if (d.data.message == "success") {
    ...
    s.btn_save_click = function (action_status) {
        try {
            if (ValidateFields()) {
                ...
                //ENDED BY JORGE
                h.post("../rSSAHRDoctor/Save", { data: data }).then(function (d) {
                    if (d.data.message == "success") {
                        s.FilterPageGrid();
    ...
    s.btn_print2 = function () {
        try {
            h.post("../rSSAHRDoctor/PreviousValuesOnPage_rSSDTROverrides").then(function (d) {
                h.post("../rSSAHRDoctor/RetriveData",
                    {
    ...
    s.UpdateAction = function (values, action) {  //ADDED BY JORGE: 2021-05-21
        ...
        //FOR EDIT UNDERTIME AND DELETE TIME IN -SINGLE UPDATE -- LAST MODIFICATION AUTOMATIC FOR OT REMARKS
