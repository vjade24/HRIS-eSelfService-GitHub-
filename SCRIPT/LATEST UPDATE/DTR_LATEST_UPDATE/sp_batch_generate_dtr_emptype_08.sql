ALTER PROCEDURE sp_batch_generate_dtr_emptype_08  
...
AS    
 ...
 ----------------------------------------------------    
  DELETE FROM dbo.empl_dtr_hdr_tmp_08    
  WHERE dtr_user_id = @p_user_id1    
    
  DELETE FROM dbo.empl_dtr_dtl_tmp_08    
  WHERE dtr_user_id = @p_user_id1    
 ----------------------------------------------------    
...
(MINUTE,CONVERT(TIME,@v_dinner_break_start),CONVERT(TIME,@d_time_out_pm_ot)) / 60.00)     
  ...
  SELECT @v_approval_status = A.approval_status,@v_check_exists_qcna = 1    
   ...
   IF @v_exists_dtr_hdr = 0    
    BEGIN    
     ...
     INSERT INTO empl_dtr_hdr_tmp_08    
     SELECT     
       @p_user_id1    
      ,@d_dtr_order_no    
      ,@p_dtr_year1    
   ...
   INSERT INTO empl_dtr_dtl_tmp_08    
   SELECT    
     @p_user_id1    
    ,@d_dtr_order_no       
    ,@d_empl_id        
 ...
 WHERE empl_dtr_dtl_tbl.empl_id IN (  
              SELECT empl_id FROM empl_dtr_hdr_tmp_08 A1     
              WHERE A1.empl_id = empl_dtr_dtl_tbl.empl_id    
     AND   A1.dtr_user_id = @p_user_id1    
