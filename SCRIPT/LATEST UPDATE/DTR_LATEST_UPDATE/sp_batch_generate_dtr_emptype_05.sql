ALTER PROCEDURE sp_batch_generate_dtr_emptype_05        
...
AS          
 ...
 ----------------------------------------------------          
  DELETE FROM dbo.empl_dtr_hdr_tmp_05          
  WHERE dtr_user_id = @p_user_id1          
          
  DELETE FROM dbo.empl_dtr_dtl_tmp_05          
  WHERE dtr_user_id = @p_user_id1          
 ----------------------------------------------------          
 ...
 DECLARE @v_table_adj_cal AS TABLE        
  ...
  ,ts_code            VARCHAR(02)         
   ...
   ,reg_pm_end_time         VARCHAR(05)         ,actual_time_in_am        VARCHAR(05)            
  ...
  LEFT JOIN empl_dtr_hdr_tbl Z          ON  Z.empl_id = A.empl_id          
...
END          
 ...
 --ENDEDD ADDED BY JORGE FOR UNDERTIME REPORT: 2022-04-19        
   ...
   --added By marvin --      
   --start--      
     DECLARE @YR VARCHAR(04)  = YEAR(@v_dtr_date)      
     DECLARE @MO VARCHAR(02)  = MONTH(@v_dtr_date)      
     DECLARE @over_time_request_in VARCHAR(05)      
     DECLARE @over_time_request_out VARCHAR(05)      
     DECLARE @over_time_request_in_ampm VARCHAR(02)      
     DECLARE @over_time_request_out_ampm VARCHAR(02)      
         
         
      SELECT       
      @over_time_request_in = ot_start_time      
     ,@over_time_request_in_ampm = ot_start_ampm      
     ,@over_time_request_out = ot_end_time      
     ,@over_time_request_out_ampm = ot_end_ampm      
     FROM func_otrqst_info_date (@YR,@MO,@p_empl_id)       
     WHERE dtr_date = @v_dtr_date      
         
     SET @over_time_request_in = @over_time_request_in      
     SET @over_time_request_in_ampm = @over_time_request_in_ampm      
     SET @over_time_request_out = @over_time_request_out      
     SET @over_time_request_out_ampm = @over_time_request_out_ampm      
         
     SET @over_time_request_in = IIF(UPPER(@over_time_request_in_ampm) = 'AM',@over_time_request_in,(SELECT FORMAT(CONVERT(DATETIME,@over_time_request_in + 'PM',101),'HH:mm','en-us')))      
     SET @over_time_request_out = IIF(UPPER(@over_time_request_out_ampm) = 'AM',@over_time_request_out,(SELECT FORMAT(CONVERT(DATETIME,@over_time_request_out + 'PM',101),'HH:mm','en-us')))      
   ...
   --SELECT @v_dtr_date, @d_time_in_am_ot,@d_time_out_am_ot,@v_time_ot_payable_am,@d_time_in_pm_ot,@d_time_out_pm_ot,@v_time_ot_payable_pm,@d_time_ot_payable  
  
   --IF CONVERT(TIME,@d_time_in_am_ot) < CONVERT(TIME,@v_reg_am_start_time)  
   --BEGIN  
   --  SET @v_time_ot_payable_am = (DATEDIFF(mi,CONVERT(TIME,@d_time_in_am_ot),CONVERT(TIME,@d_time_out_am_ot)) - 60)/60.00  
   --  IF (DATEDIFF(mi,CONVERT(TIME,@d_time_in_am_ot),CONVERT(TIME,@d_time_out_am_ot)) - 60) < 1  
   --  BEGIN   
   --     SET @v_time_ot_payable_am = 0.00  
   --  END  
   --END  
   SET @d_time_ot_payable   = @v_time_ot_payable_am + @v_time_ot_payable_pm          
         ...
         -- MARVIN    
    --EXEC sp_batch_generate_dtr_emptype_05 '2022','10','2761','JO','05','BTHADMN'          
   ...
   IF (ISNULL(@over_time_request_in,'') <> '' AND DATEDIFF(mi,CONVERT(TIME,@over_time_request_in),CONVERT(TIME,@v_reg_am_start_time)) > 0)      
    AND (ISNULL(@over_time_request_out,'') <> '' AND DATEDIFF(mi,CONVERT(TIME,@v_reg_pm_end_time),CONVERT(TIME,@over_time_request_out)) > 0)      
   BEGIN      
     ...
     DECLARE @TIME_0T_AM INT -- TIME DIFFERENCE IF ACTUAL TIME IN IS LESS THAN BREAKFAST TIME START        
     DECLARE @TIME_0T_PM INT -- TIME DIFFERENCE IF ACTUAL TIME IN IS GREATER THAN BREAKFAST TIME START         
     SET @TIME_0T_AM = IIF((DATEDIFF(mi,@v_actual_time_in_am,@v_reg_am_start_time)-60) < 1,0,DATEDIFF(mi,@v_actual_time_in_am,@v_reg_am_start_time)-60)      
    -- SET @TIME_0T_PM = IIF(DATEDIFF(mi,@v_reg_pm_end_time,@v_actual_time_out_pm) < 120 and DATEDIFF(mi,@v_reg_pm_end_time,@v_actual_time_out_pm) < 120,DATEDIFF(mi,@v_reg_pm_end_time,@v_actual_time_out_pm),DATEDIFF(mi,@v_reg_pm_end_time,@v_actual_time_out_pm)-60)      
     ...
     WHEN DATEDIFF(mi,@v_reg_pm_end_time,@v_actual_time_out_pm) > 180  AND DATEDIFF(mi,@v_reg_pm_end_time,@v_actual_time_out_pm) <= 240 --if greater than 3 hours but less than 4 hours    
     THEN 180    
     WHEN DATEDIFF(mi,@v_reg_pm_end_time,@v_actual_time_out_pm) >=240  -- if greater or equals than 4 hours     
     THEN (DATEDIFF(mi,@v_reg_pm_end_time,@v_actual_time_out_pm)-60)    
     ELSE 0    
     END    
     )    
      END    
      ...
      BEGIN     
     SET @TIME_0T_PM = (SELECT CASE     
     WHEN DATEDIFF(mi,@v_reg_pm_end_time,@over_time_request_out) = 180 -- if less than 3 hours    
     THEN DATEDIFF(mi,@v_reg_pm_end_time,@over_time_request_out)    
     WHEN DATEDIFF(mi,@v_reg_pm_end_time,@over_time_request_out) = 240 -- IF 4 HOURS    
     THEN 180    
     WHEN DATEDIFF(mi,@v_reg_pm_end_time,@over_time_request_out) = 300 -- IF 5 HOURS    
     THEN 240    
     WHEN DATEDIFF(mi,@v_reg_pm_end_time,@over_time_request_out) = 360 -- IF 5 HOURS    
     THEN 300    
     ELSE 0    
     END    
     )    
   ...
   IF @v_late_no_ot = 1 --CHECK IF OVERTIME IS COUNTED IF THERE IS AN UNDERTIME          
     ...
     END          
         ...
         IF (ISNULL(@over_time_request_in,'') <> '' AND DATEDIFF(mi,CONVERT(TIME,@over_time_request_in),CONVERT(TIME,@v_reg_am_start_time)) > 0)      
    AND (ISNULL(@over_time_request_out,'') <> '' AND DATEDIFF(mi,CONVERT(TIME,@v_reg_pm_end_time),CONVERT(TIME,@over_time_request_out)) > 0)   
    AND (@v_cal_date_type NOT IN ('HOL','SUN','SAT') OR @v_ts_code = '98')  
      
   BEGIN   
         SET @v_time_ot_minute = (@TIME_0T_AM + @TIME_0T_PM)       
         SET @d_time_ot_payable = (@v_time_ot_minute/60)      
   ...
   BEGIN  
     ...
     -- sp_batch_generate_dtr_emptype '2021','02','2886','JO','03','BTHADMN'          
      ...
      SET @v_time_ot_minute  = @v_time_ot_minute - IIF(DATEDIFF(MINUTE,CONVERT(TIME,@v_dinner_break_start),CONVERT(TIME,@d_time_out_pm_ot)) / 60.00 > 1,DATEDIFF(MINUTE,CONVERT(TIME,@v_dinner_break_start),CONVERT(TIME,@v_dinner_break_end)),DATEDIFF(MINUTE,
CONVERT(TIME,@v_dinner_break_start),CONVERT(TIME,@d_time_out_pm_ot)))           
     END          
   ...
   ELSE          
    BEGIN          
     SET @t_remarks_OT = ''          
    END          
 --   IF @v_dtr_date ='2022-10-22'  
 --BEGIN  
 --    SELECT @v_dtr_date,@d_time_ot_hris  
 --   END      
  ...
  SELECT @v_approval_status = A.approval_status,@v_check_exists_qcna = 1          
   ...
   IF @v_exists_dtr_hdr = 0          
    BEGIN          
     ...
     INSERT INTO empl_dtr_hdr_tmp_05          
     SELECT           
       @p_user_id1          
      ,@d_dtr_order_no          
      ,@p_dtr_year1          
   ...
   INSERT INTO empl_dtr_dtl_tmp_05          
   SELECT          
     @p_user_id1          
    ,@d_dtr_order_no             
    ,@d_empl_id              
 ...
 WHERE empl_dtr_dtl_tbl.empl_id IN (        
              SELECT empl_id FROM empl_dtr_hdr_tmp_05 A1           
              WHERE A1.empl_id = empl_dtr_dtl_tbl.empl_id          
     AND   A1.dtr_user_id = @p_user_id1          
