...
ALTER PROCEDURE sp_batch_generate_dtr_emptype_02_reg        
...
AS          
 ...
 ----------------------------------------------------          
  DELETE FROM dbo.empl_dtr_hdr_tmp_02_reg          
  WHERE dtr_user_id = @p_user_id1          
          
  DELETE FROM dbo.empl_dtr_dtl_tmp_02_reg          
  WHERE dtr_user_id = @p_user_id1          
 ----------------------------------------------------          
 ...
 --ENDEDD ADDED BY JORGE FOR UNDERTIME REPORT: 2022-04-19        
   ...
   IF (@v_ts_code = '0' OR @v_cal_date_type IN ('HOL','SUN','SAT')) AND @v_otr_exist = 'Y'          
    BEGIN          
     ...
     IF CONVERT(TIME,@d_time_out_am) > CONVERT(TIME,@v_reg_am_end_time) -- SET AM TIME OUT TO OT REQUEST AM TIME OUT IF  AM TIME OUT IS GREATER THAN OT AM TIME OUT REQUEST         
     BEGIN          
      SET @d_time_out_am_ot = @v_reg_am_end_time          
     END          
          
     IF CONVERT(TIME,@d_time_in_pm) < CONVERT(TIME,@v_reg_pm_start_time)  -- SET PM TIME IN TO OT REQUEST PM TIME IN IF  PM TIME IN IS LESS THAN OT PM TIME IN REQUEST        
     BEGIN          
     ...
     IF CONVERT(TIME,@d_time_out_pm) > CONVERT(TIME,@v_OTR_e_time) --SET VALUE INTO END TIME OF OVERTIME REQUEST: JORGE RUSTOM VILLANUEVA          
      ...
      -- SELECT @v_dtr_date, @d_time_in_am_ot,@d_time_out_am_ot,@v_time_ot_payable_am,@d_time_in_pm_ot,@d_time_out_pm_ot,@v_time_ot_payable_pm,@d_time_ot_payable      
         -- MARVIN        
    --sp_batch_generate_dtr_emptype_02_reg '2022','10','2761','JO','05','BTHADMN'                  
     END          
   ...
   ELSE --CALCULATE WEEKDAYS OVERTIME          
    BEGIN          
       ...
       IF @v_late_no_ot = 1 --CHECK IF OVERTIME IS COUNTED IF THERE IS AN UNDERTIME          
        ...
        -- SELECT @v_dtr_date,@d_time_ot_payable ,@v_reg_pm_end_time,@d_time_out_pm_ot,@v_OTR_e_time      
    END          
...
(MINUTE,CONVERT(TIME,@v_dinner_break_start),CONVERT(TIME,@d_time_out_pm_ot)) / 60.00)           
 ...
 -- MARVIN       
   ...
   --SELECT  @v_dtr_date,@d_time_ot_payable, @v_time_ot_payable_minimum      
 ...
 --IF @v_dtr_date = '2022-10-22'      
 --BEGIN      
 --  select @v_dtr_date, @t_remarks_OT      
 --END      
   -- select @v_dtr_date, @t_remarks_OT      
    ...
    BEGIN          
     ...
     BEGIN          
      ...
      BEGIN          
             ...
             SET @d_time_in_am = ''          
             SET @d_time_out_am = ''                 SET @t_remarks_WFH = 'WORK FROM HOME'          
             SET @d_remarks_details    ='WORK FROM HOME'          
             SET @d_remarks_details_half_day =''          
            END          
           ELSE --REMOVE UNDERTIME FOR MORNING CONSIDERED WORK FROM HOME          
 ...
 AND  @v_leavetype_descr = ''          
  ...
  END        
     ...
     IF @v_ts_code <> '0' AND @v_cal_date_type <> 'HOL'          
      BEGIN          
       SET @d_remarks_details = UPPER(@t_remarks_AS)          END          
 ...
 --SELECT @d_remarks_details        
  ...
  SELECT @v_approval_status = A.approval_status,@v_check_exists_qcna = 1          
   ...
   ELSE           
    BEGIN          
      SET @d_dtr_order_no = ISNULL((SELECT TOP 1              
       A.dtr_order_no              
      FROM empl_dtr_hdr_tmp_02_reg A            
   ...
   IF @v_exists_dtr_hdr = 0          
    BEGIN          
     ...
     INSERT INTO empl_dtr_hdr_tmp_02_reg          
     SELECT           
       @p_user_id1          
      ,@d_dtr_order_no          
      ,@p_dtr_year1          
   ...
   INSERT INTO empl_dtr_dtl_tmp_02_reg          
   SELECT          
     @p_user_id1          
    ,@d_dtr_order_no             
    ,@d_empl_id              
  ...
  WHERE EXISTS (SELECT 'X'          
          FROM empl_dtr_hdr_tmp_02_reg A1           
 ...
 WHERE empl_dtr_dtl_tbl.empl_id IN (        
              SELECT empl_id FROM empl_dtr_hdr_tmp_02_reg A1           
              WHERE A1.empl_id = empl_dtr_dtl_tbl.empl_id          
     AND   A1.dtr_user_id = @p_user_id1          
  ...
  SELECT           
   ...
   ,no_of_ob              
   ,no_of_lv              
  FROM empl_dtr_dtl_tmp_02_reg A          
  WHERE A.dtr_user_id = @p_user_id1         
  --AND          
  --NOT EXISTS (SELECT 'X' FROM empl_dtr_hdr_tbl A1          
  --    WHERE A1.empl_id = A.empl_id          
