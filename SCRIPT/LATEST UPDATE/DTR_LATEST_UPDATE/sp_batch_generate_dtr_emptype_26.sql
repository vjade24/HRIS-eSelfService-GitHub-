ALTER PROCEDURE sp_batch_generate_dtr_emptype_26      
...
AS        
 ...
 ----------------------------------------------------        
  DELETE FROM dbo.empl_dtr_hdr_tmp_26        
  WHERE dtr_user_id = @p_user_id1        
        
  DELETE FROM dbo.empl_dtr_dtl_tmp_26        
  WHERE dtr_user_id = @p_user_id1        
 ----------------------------------------------------        
 ...
 DECLARE @v_table_adj_cal AS TABLE      
  ...
  ,@v_to_time_out_pm          = to_time_out_pm      ,@v_to_remarks              = to_remarks       
...
END        
 ...
 --ENDEDD ADDED BY JORGE FOR UNDERTIME REPORT: 2022-04-19      
  ...
  END            
   ...
   -- SET @d_time_out_pm_ot = @v_OTR_e_time          
    ...
    SET @d_time_ot_payable = DATEDIFF(mi,CONVERT(TIME,@v_reg_pm_end_time),CONVERT(TIME,@d_time_out_pm))/60.00        
       SET @v_time_ot_minute  = DATEDIFF(mi,CONVERT(TIME,@v_reg_pm_end_time),CONVERT(TIME,@d_time_out_pm))        
       --SELECT @d_under_Time,@v_dtr_date      
    --select @v_dtr_date as v_dtr_date, @d_time_ot_payable as d_time_ot_payable_1 , @v_time_ot_payable_minimum AS v_time_ot_payable_minimum, @d_time_ot_hris AS d_time_ot_hris      
  ...
  SELECT @v_approval_status = A.approval_status,@v_check_exists_qcna = 1        
   ...
   IF @v_exists_dtr_hdr = 0        
    BEGIN        
     ...
     INSERT INTO empl_dtr_hdr_tmp_26        
     SELECT         
       @p_user_id1        
      ,@d_dtr_order_no        
      ,@p_dtr_year1        
   ...
   INSERT INTO empl_dtr_dtl_tmp_26        
   SELECT        
     @p_user_id1        
    ,@d_dtr_order_no           
    ,@d_empl_id            
 ...
 WHERE empl_dtr_dtl_tbl.empl_id IN (      
              SELECT empl_id FROM empl_dtr_hdr_tmp_26 A1         
              WHERE A1.empl_id = empl_dtr_dtl_tbl.empl_id        
     AND   A1.dtr_user_id = @p_user_id1        
