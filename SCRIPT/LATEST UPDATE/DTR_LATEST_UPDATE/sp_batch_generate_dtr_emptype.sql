-- EXEC sp_batch_generate_dtr_emptype '2023','05','6136','CE','26','BTHDAMIN'

ALTER PROCEDURE [dbo].[sp_batch_generate_dtr_emptype]      
...
AS        
 ...
 ----------------------------------------------------        
  DELETE FROM dbo.empl_dtr_hdr_tmp        
  WHERE dtr_user_id = @p_user_id1        
        
  DELETE FROM dbo.empl_dtr_dtl_tmp        
  WHERE dtr_user_id = @p_user_id1        
 ----------------------------------------------------        
  ...
  DECLARE @v_shift_flag   VARCHAR(01)  -- ADDED BY MARVIN       
 ...
 DECLARE @v_table_adj_cal AS TABLE      
  ...
  ,ts_code            VARCHAR(02)       
   ...
   ,shift_flag              VARCHAR(01)  -- ADDED BY MARVIN   
   ,as_exist    VARCHAR(01)      
...
---  select * from @v_table_adj_cal
  ...
  ,@v_shift_flag     =  shift_flag     -- ADDED BY MARVIN    
  ,@v_as_exist                = as_exist     
 ...
 END      
  ...
  -- ADDED BY MARVIN   
  ----********************************************************-------  
  ----- EARLY TIME OUT ADJUSTMENT ON MARCH 07, 2023 DURING EARTHQUAKE   
  -----*******************************************************-------  
  
  IF @v_dtr_date = '2023-03-07' AND @v_shift_flag = '1'  
  BEGIN  
         
   IF ISNULL(@v_time_in_pm,'') <> ''  
   BEGIN  
       IF CONVERT(TIME,@v_actual_time_out_pm) < '17:00'   
    BEGIN   
        ...
        ELSE --CALCULATE UNDERTIME FOR REGULAR SCHEDULE ADDED BY JORGE: 01-11-2021        
           BEGIN        
            ...
            IF @d_time_out_pm <> '' AND CONVERT(time,@d_time_out_pm) < CONVERT(time,@v_reg_pm_end_time) AND @v_check_two_entries_undertime = 1        
             ...
             IF @v_under_Time_minute_total_remarks > 0 OR @v_under_Time_hrs_total_remarks > 0                  BEGIN        
 ...
 --ADDEDD BY MARVIN   
     IF CONVERT(TIME,@v_OTR_e_time) = '00:00'    
  BEGIN  
     SET  @v_OTR_e_time = '23:59'  
  END             
   ...
   -- SELECT  @v_time_ot_minute AS time_ot_minute, @d_time_in_am_ot AS time_in_am_ot    
          -- ex             ec sp_batch_generate_dtr_emptype '2022','10','7293','JO','05','BTHADMIN'    
       END     
 ...
 ---- added by marvin -- start---    
        
    SET @d_time_out_pm  = IIF(ISNULL(@v_pre_time_in_hrs,'') <> '' AND @d_time_out_pm > @v_OTR_e_time,@v_OTR_e_time,@d_time_out_pm)     
    IF CONVERT(TIME,@d_time_out_pm) > CONVERT(TIME,@v_reg_pm_end_time) --CHECK IF EMPLOYEE RENDERED MORE THAN SHIFT PM OUT        
  ...
  SET @d_time_out_pm_ot = @v_OTR_e_time          
   ...
   --  SELECT @v_reg_pm_end_time AS v_reg_pm_end_time,@d_time_out_pm_ot AS d_time_out_pm_ot  
         
     
     SET @d_time_ot_payable = DATEDIFF(mi,CONVERT(TIME,@v_reg_pm_end_time),CONVERT(TIME,@d_time_out_pm))/60.00     --change @d_time_out_pm_ot to  @d_time_out_pm     
     SET @v_time_ot_minute  = DATEDIFF(mi,CONVERT(TIME,@v_reg_pm_end_time),CONVERT(TIME,@d_time_out_pm))        
  ...
  ------ added by marvin -- end---    
       ...
       IF @v_late_no_ot = 1 --CHECK IF OVERTIME IS COUNTED IF THERE IS AN UNDERTIME        
        BEGIN        
         IF @d_under_Time > 0        
          ...
          END        
        END        
         -- SELECT @v_time_ot_minute    
   -- exec sp_batch_generate_dtr_emptype '2022','10','7293','JO','05','BTHADMIN'    
   ...
   AND  CONVERT(TIME,@v_OTR_s_time) < @d_time_in_am_ot      
    BEGIN    
 -- SELECT  @v_time_ot_minute AS v_time_ot_minute , @d_time_ot_payable AS d_time_ot_payable,@d_time_in_am_ot AS d_time_in_am_ot,@v_breakfast_break_start AS v_breakfast_break_start,@v_breakfast_break_end AS v_breakfast_break_end        
  ...
  SET @v_time_ot_minute  = @v_time_ot_minute - IIF(DATEDIFF(MINUTE,CONVERT(TIME,@d_time_in_am_ot),CONVERT(TIME,@v_breakfast_break_end)) > 1,DATEDIFF(MINUTE,CONVERT(TIME,@v_breakfast_break_start),CONVERT(TIME,@v_breakfast_break_end)),DATEDIFF(MINUTE,CONVERT(TIME,@d_time_in_am_ot),CONVERT(TIME,@v_breakfast_break_end)))      
    ...
    --  SELECT @v_dtr_date AS dtr_date,@d_time_in_am_ot AS time_in_am_ot,@d_time_out_pm_ot AS time_out_pm_ot,@v_breakfast_break_end AS breakfast_break_end, @d_time_ot_payable AS time_ot_payable, @v_time_ot_minute AS time_ot_minute, 'AM' AS period    
   -- exec sp_batch_generate_dtr_emptype '2022','10','7293','JO','05','BTHADMIN'    
  --  select     
  --    @v_dtr_date    
  -- ,@d_time_ot_payable    
 ...
 --  select @v_dtr_date as v_dtr_date, @d_time_ot_payable as d_time_ot_payable_1    
   ...
   -- SELECT @v_dtr_date as v_dtr_date, @t_remarks_OT AS t_remarks_OT    
    --  SELECT @t_remarks_OT AS t_remarks_OT    
   --------- COMPUTE OT ------------        
   -------------------------------------------------------------------------        
...
END        
          ...
          IF CONVERT(TIME,@d_time_in_pm) >= CONVERT(TIME,@v_reg_pm_start_time)        
            ...
            BEGIN        
             ...
             SET @v_under_Time_hrs_total =  IIF(@v_under_Time_hrs_pm_in < 0,0,@v_under_Time_hrs_pm_in)        
               ...
               IF @v_under_Time_minute_total_remarks > 0 OR @v_under_Time_hrs_total_remarks > 0        
                BEGIN        
                 SET @d_under_Time_remarks = IIF(@v_under_Time_hrs_total_remarks > 0,CONVERT(VARCHAR(02),@v_under_Time_hrs_total_remarks) + 'h ' + IIF(CONVERT(INT,@v_under_Time_minute_total_remarks) > 0,CONVERT(VARCHAR(02),CONVERT(INT,@v_under_Time_minute_total_remarks)) + 'm',''),CONVERT(VARCHAR(02),CONVERT(INT,@v_under_Time_minute_total_remarks)) + 'm')                    END        
 ...
 AND  @v_leavetype_descr = ''        
  ...
  IF @v_remarks_pt <> ''        
  BEGIN        
   SET @d_remarks_details = REPLACE(@d_remarks_details,'?','')      
  ...
  --EXEC sp_batch_generate_dtr_emptype '2022','08','9229','RE','04','U9229'      
 ...
 WHERE empl_dtr_dtl_tbl.empl_id IN (      
              SELECT empl_id FROM empl_dtr_hdr_tmp A1         
              WHERE A1.empl_id = empl_dtr_dtl_tbl.empl_id        
     AND   A1.dtr_user_id = @p_user_id1        
  ...
  SELECT         
   ...
   ,no_of_ob            
   ,no_of_lv            
  FROM empl_dtr_dtl_tmp A        
  WHERE A.dtr_user_id = @p_user_id1 
  
  
  --SELECT * FROM empl_dtr_dtl_tmp      
